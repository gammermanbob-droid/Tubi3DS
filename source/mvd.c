/*
  ...
  VENDORED + PATCHED copy of libctru's mvd.c (zlib license).
  This file is compiled into 3dsfin and OVERRIDES libctru's mvd module: because
  our object defines every mvd* symbol, the linker never pulls libctru's mvd.o
  from libctru.a, so this whole file (including its static state) is used instead.

  The ONLY change vs stock libctru is in mvdstdRenderVideoFrame: SetConfig is now
  guarded by `if(config)` so passing NULL renders WITHOUT re-running SetConfig.
  Re-running SetConfig after mvdstdProcessVideoFrame breaks the decode->render
  handoff (ControlFrameRendering returns OK but writes no pixels), which is why
  stock libctru can't drive MVD playback. Patch matches Core-2-Extreme/libctru_custom.
  See project memory (project_3dsfin.md) for the full diagnosis.
*/
